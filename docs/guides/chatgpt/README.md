...
1. **Ship MCP 2.0** (PR #40) with its operator settings from SPLIT-1420: `MCP_OAUTH_SIGNING_KEY`, `MCP_PUBLIC_URL=https://mcp-server-splitmygear.vercel.app`, `MCP_BFF_RELAY_KEY` and the Upstash store.
...
- **Sign-in may stall on the last redirect.** The sign-in page's CSP includes `form-action 'self'` (added 2026-09-06). A password or code form then ends in a 302 to `https://chatgpt.com/...`. Chromium applies `form-action` to redirects that follow a form submission. No real browser has driven that path since the directive was added.
- **The ChatGPT flow is untested.** 2.0 has never been exercised end to end from ChatGPT. The branch documents Claude, Cursor, Windsurf and the MCP Inspector, and does not mention ChatGPT.
  ...
  - Any scope value outside the 12 Splitt scopes, such as `offline_access`, returns `invalid_scope`.
