...
- **Your password is never stored.** The hosted sign-in page relays it once to Splitt's login API, together with your real IP via the backend's trusted relay header (`MCP_BFF_RELAY_KEY`) so Splitt's brute-force throttling keys on you, not on this server. The form only accepts same-origin submissions (a hostile page cannot drive it with its visitors' browsers). A rejected password is answered with one fixed message that says nothing about whether the address is registered.
- **Sign-in throttling, and what it deliberately does not do.** Failed attempts are counted per IP and per account, 10 per 10 minutes; the count is consumed atomically as part of the check, so a burst of simultaneous guesses cannot slip past the ceiling together. A successful sign-in gives its slot back and never resets the budget. The two counters are enforced at different moments, on purpose: the **per-IP** one refuses an attempt up front, while the **per-account** one is consulted only *after* the backend has already rejected the credentials. That ordering is what stops a stranger who knows your email address from locking you out of MCP sign-in by submitting wrong passwords for you (CWE-645) — your correct password returns before the account counter is ever read. The trade-off is real and worth stating: the per-account counter no longer keeps a password-spray *away* from the backend, only from succeeding. Splitt's own per-IP and per-email throttles — which see your real address, not this server's — are the authoritative control, and they use the same ordering (SPLIT-427).
...
- **Phishing resistance.** Registration is closed by default: an `https` redirect URI may register **only** if its host is on the operator allow-list (`MCP_OAUTH_ALLOWED_REDIRECT_HOSTS`), so an unset variable permits nothing rather than everything, and registration fails with an error naming the variable. Loopback (`http://localhost`, `127.0.0.1`, `[::1]`) is always allowed. The list is re-checked on every use of a client id, not just at registration, so tightening it immediately invalidates ids already issued under a looser policy. The sign-in page still shows the app's name and the exact address it will send you to, and labels the app "unverified" when its redirect host is not allow-listed. Proxy headers are only trusted on Vercel or with `MCP_TRUST_PROXY_HEADERS=1`.
...
- **Rate limiting: two budgets, charged per invocation.** Every principal has a per-minute budget of HTTP requests AND a separate one of tool invocations (SPLIT-1449). A JSON-RPC batch is charged per `tools/call` member, all-or-nothing — one POST cannot buy N invocations for the price of one, and a refused batch consumes nothing — and the 429 says which ceiling was hit. Both budgets share one principal key (verified user id, or the caller's address on the operator key) in disjoint key spaces, so neither can exhaust the other.
- **Rate limiting and throttles across instances.** With a shared store configured (`UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN`, or the Vercel KV names), the per-principal MCP rate limit, the sign-in failure throttle (10 failures per 10 minutes per IP and per account) and the authorization-code single-use cache are enforced across every serverless instance.
  **Without one they are per-instance — which on Vercel means per concurrent lambda, and a distributed attacker simply outruns them.** That degradation is allowed but never silent: each affected path logs a startup warning naming all three protections, once per instance. It is allowed rather than fatal because failing closed on a paid dependency would take the only user-facing sign-in path offline, while the two controls that actually stand between an attacker and an account — Splitt's own throttles (fed your real IP) and mandatory PKCE S256 on every authorization code — are unaffected by it. An operator who wants the strict posture instead sets `MCP_REQUIRE_SHARED_STORE=1`, and OAuth then refuses to enable at all until a store is configured.
